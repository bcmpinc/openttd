...
/** Find out how the grid should be drawn. */
void SmallMapWindow::RecalcGridProps()
{
	this->grid_size = BASE_GRID_SIZE;

	/* Size of the grid can be finetuned by the smallmap_grid_size setting. To have a nice
	 * exponential growth, multiply by 1.5 every smallmap_grid_size step. Exact mutiplier
	 * for n-th size is: 1.5^(n-1) = 3^(n-1) / 2^(n-1) */
	for (uint i = 1; i < _settings_client.gui.smallmap_grid_size; i++) this->grid_size *= 3; // multiply by 3^(n-1)
	this->grid_size >>= (_settings_client.gui.smallmap_grid_size - 1); // divide by 2^(n-1)

	/* Multiply by the zoom level rounded up to the nearrest power of 2. Rounding
	 * is done to keep lines in the same place when zoomming in or out. */
	this->grid_size *= RoundUpPow2(this->zoom);
